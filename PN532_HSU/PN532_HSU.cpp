...
PN532_HSU::PN532_HSU(IHardwareSerial &serial) {
  _serial = &serial;
  command = 0;
}

void PN532_HSU::begin() { _serial->begin(); }
...
int8_t PN532_HSU::receive(uint8_t *buf, int len, uint16_t timeout) {
  ...
  while (read_bytes < len) {
    start_millis = k_uptime_get();
    do {
      ret = _serial->read(&c);
      if (ret == 0) {
        break;
      }
    } while ((timeout == 0) || ((k_uptime_get() - start_millis) < timeout));
